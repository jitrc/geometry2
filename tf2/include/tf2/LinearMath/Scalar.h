// Copyright 2003-2009 Erwin Coumans  http://bullet.googlecode.com
//
...
// non-windows systems
  #define TF2SIMD_FORCE_INLINE inline
// TODO(anyone): check out alignment methods for other platforms/compilers
// #define ATTRIBUTE_ALIGNED16(a) a __attribute__ ((aligned (16)))
// #define ATTRIBUTE_ALIGNED64(a) a __attribute__ ((aligned (64)))
// #define ATTRIBUTE_ALIGNED128(a) a __attribute__ ((aligned (128)))
  #define ATTRIBUTE_ALIGNED16(a) a
...
#define tf2RecipSqrt(x) ((tf2Scalar)(tf2Scalar(1.0) / tf2Sqrt(tf2Scalar(x))))
  ...
  float valueIfConditionZero)
{
  ...
  return (float)tf2Fsel(
    (tf2Scalar)condition - tf2Scalar(
...
TF2SIMD_FORCE_INLINE uint8_t tf2SwapEndian(uint8_t val)
{
  return static_cast<uint8_t>(((val & 0xff00) >> 8) | ((val & 0x00ff) << 8));
}
...
TF2SIMD_FORCE_INLINE uint8_t tf2SwapEndian(int8_t val)
{
  return tf2SwapEndian((uint8_t) val);
}

// tf2SwapFloat uses using char pointers to swap the endianness
// tf2SwapFloat/tf2SwapDouble will NOT return a float, because the machine might 'correct'
// invalid floating point values
// Not all values of sign/exponent/mantissa are valid floating point numbers according to IEEE 754.
// When a floating point unit is faced with an invalid value, it may actually change the value,
// or worse, throw an exception.
// In most systems, running user mode code, you wouldn't get an exception,
// but instead the hardware/os/runtime will 'fix' the number for you.
...
{
  explicit tf2TypedObject(int objectType)
