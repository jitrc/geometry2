...
public:
  ...
  {
    m_floats[0] += q.x(); m_floats[1] += q.y(); m_floats[2] += q.z();
  ...
  * @param q The other quaternion */
  ...
  /**@brief Return the angle [0, Pi] of rotation represented by this quaternion along
     the shortest path */
  TF2_PUBLIC
  ...
  Vector3 getAxis() const
  {
    ...
    if (s_squared < tf2Scalar(10.) * TF2SIMD_EPSILON) {
      // Arbitrary
  ...
  {
    static const Quaternion identityQuat(
      tf2Scalar(0.), tf2Scalar(0.), tf2Scalar(0.),
      tf2Scalar(1.));
    return identityQuat;
  }
...
shortestArcQuat(const Vector3 & v0, const Vector3 & v1)
{
  ...
  if (d < -1.0 + TF2SIMD_EPSILON) {
    ...
    return Quaternion(n.x(), n.y(), n.z(), 0.0f);
  }
