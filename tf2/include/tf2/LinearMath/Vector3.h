 ...
 * @param The vector to add to this one */
  ...
  {
    return m_floats[0] * v.m_floats[0] + m_floats[1] * v.m_floats[1] + m_floats[2] *
           v.m_floats[2];
  }
  ...
  {
    return m_floats[0] <
           m_floats[1] ? (m_floats[0] < m_floats[2] ? 0 : 2) : (m_floats[1] <
           m_floats[2] ? 1 : 2);
  }
  ...
  {
    return m_floats[0] <
           m_floats[1] ? (m_floats[1] < m_floats[2] ? 2 : 1) : (m_floats[0] <
           m_floats[2] ? 2 : 0);
  }
...
*/
  ...
  {
    return m_floats[0] == tf2Scalar(0) && m_floats[1] == tf2Scalar(0) && m_floats[2] == tf2Scalar(
      0);
  }
...
{
  return Vector3(
    v1.m_floats[0] + v2.m_floats[0], v1.m_floats[1] + v2.m_floats[1],
...
TF2SIMD_FORCE_INLINE void Vector3::serializeFloat(struct  Vector3FloatData & dataOut) const
{
  ...
  for (int i = 0; i < 4; i++) {
    dataOut.m_floats[i] = static_cast<float>(m_floats[i]);
  }
}
...
TF2SIMD_FORCE_INLINE void Vector3::serializeDouble(struct Vector3DoubleData & dataOut) const
{
  // could also do a memcpy, check if it is worth it
  for (int i = 0; i < 4; i++) {
    dataOut.m_floats[i] = static_cast<double>(m_floats[i]);
  }
}
